defmodule KatzenWeb.CoreComponents do
  @moduledoc """
    ...
    <div phx-feedback-for={@name}>
      <label class="flex gap-4 items-center text-sm leading-6 text-zinc-600">
        ...
        <input
          ...
          value="true"
          checked={@checked}
          class="rounded focus:ring-0 border-zinc-300 text-zinc-900"
          {@rest}
        />
        <%= @label %>
      </label>
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="block mt-1 w-full bg-white rounded-md border border-gray-300 shadow-sm sm:text-sm focus:ring-0 focus:border-zinc-400"
        multiple={@multiple}
        {@rest}
      >
          ...
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-zinc-50">
            ...
            >
              <div class="block py-4 pr-6">
                <span class="absolute right-0 -left-4 -inset-y-px sm:rounded-l-xl group-hover:bg-zinc-50" />
            ...
            <td :if={@action != []} class="relative p-0 w-14">
              <div class="relative py-4 text-sm font-medium text-right whitespace-nowrap">
                <span class="absolute left-0 -right-4 -inset-y-px sm:rounded-r-xl group-hover:bg-zinc-50" />
                <span
                  :for={action <- @action}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(KatzenWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(KatzenWeb.Gettext, "errors", msg, opts)
    end
  end
